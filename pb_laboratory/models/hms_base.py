...
class AccountInvoice(models.Model):
    ...
    request_id = fields.Many2one('pb.laboratory.request', string='Lab Request', copy=False, ondelete='restrict')
...
class PBConsumableLine(models.Model):
    ...
    lab_test_id = fields.Many2one('pb.lab.test', string="Lab Test", ondelete="restrict")
...
class PBPatient(models.Model):
    ...
    request_ids = fields.One2many('pb.laboratory.request', 'patient_id', string='Lab Requests')
    ...
    def action_lab_requests(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_laboratory.hms_action_lab_test_request")
...
class Physician(models.Model):
    ...
    def _pb_lab_rec_count(self):
        Labrequest = self.env['pb.laboratory.request']
    ...
    lab_request_count = fields.Integer(compute='_pb_lab_rec_count', string='# Lab Request')
    lab_result_count = fields.Integer(compute='_pb_lab_rec_count', string='# Lab Result')
...
class ResUsers(models.Model):
    ...
    default_collection_center_id = fields.Many2one('pb.laboratory',  string='Collection Center', user_writeable=True)
...
class IrAttachment(models.Model):
    ...
    def _pb_portal_ensure_token(self):
