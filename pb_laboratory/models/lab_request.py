...
class PatientLabTestLine(models.Model):
    ...
    @api.depends('test_id')
    def _pb_is_manager(self):
        is_manager = self.env.user.has_group('pb_laboratory.group_hms_lab_manager')
        for rec in self:
            rec.is_manager = is_manager

    test_id = fields.Many2one('pb.lab.test',string='Test', ondelete='cascade', required=True)
    pb_tat = fields.Char(related='test_id.pb_tat', string='Turnaround Time', readonly=True)
    instruction = fields.Char(string='Special Instructions')
    request_id = fields.Many2one('pb.laboratory.request',string='Lines', ondelete='cascade')
    ...
    is_manager = fields.Boolean(compute=_pb_is_manager)
...
class LaboratoryRequest(models.Model):
    ...
    sample_ids = fields.One2many('pb.patient.laboratory.sample', 'request_id', string='Test Samples')
    laboratory_group_id = fields.Many2one('laboratory.group', ondelete="set null", string='Test Group')
    pb_laboratory_invoice_policy = fields.Selection(related="company_id.pb_laboratory_invoice_policy")

    LABSTATES = {'cancel': [('readonly', True)], 'done': [('readonly', True)]}

    other_laboratory = fields.Boolean(string='Send to Other Laboratory')
    collection_center_id = fields.Many2one('pb.laboratory', string='Collection Center', default=_get_default_collection_center_id)
    laboratory_id = fields.Many2one('pb.laboratory', ondelete='restrict', string='Laboratory')
    ...
    invoice_count = fields.Integer(compute='_pb_rec_count', string='# Invoices')

    @api.depends('name', 'patient_id.name')
    ...
    def button_requested(self):
        ...
        self.name = self.env['ir.sequence'].next_by_code('pb.laboratory.request')
    ...
    def create_sample(self):
        Sample = self.env['pb.patient.laboratory.sample']
        ...
        for line in self.line_ids:
            if line.test_id.sample_type_id:
                ...
                if (not sample_exist) or (line.test_id.pb_use_other_test_sample!=True):
    ...
    def button_accept(self):
        ...
        if company_id.pb_laboratory_invoice_policy=='in_advance':
            if not self.invoice_id:
                raise UserError(_('Invoice is not created yet.'))
            elif self.invoice_id and company_id.pb_check_laboratory_payment and self.payment_state not in ['in_payment','paid']:
                raise UserError(_('Invoice is not Paid yet.'))
        if self.sudo().company_id.pb_auto_create_lab_sample:
            self.create_sample()
        self.state = 'accepted'
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(pb_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data={'hospital_invoice_type': 'laboratory','physician_id': self.physician_id and self.physician_id.id or False})
    ...
    def create_laboratory_bill(self):
        ...
        bill = self.pb_create_invoice(partner=self.laboratory_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
        self.lab_bill_id = bill.id
        bill.request_id = self.id

    def view_invoice(self):
        action = self.pb_action_view_invoice(self.invoice_ids)
        return action

    def action_view_test_results(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_laboratory.action_lab_result")
    ...
    def action_view_lab_samples(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_laboratory.action_pb_patient_laboratory_sample")
        action['domain'] = [('request_id','=',self.id)]
    ...
    def action_sendmail(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('pb_laboratory.pb_lab_req_email', raise_if_not_found=False)
        ctx = {
            'default_model': 'pb.laboratory.request',
    ...
    def _pb_get_report_base_filename(self):
