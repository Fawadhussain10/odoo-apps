...
class PatientLabTest(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'pb.hms.mixin', 'portal.mixin', 'pb.documnt.mixin', 'pb.qrcode.mixin']
    _description = "Patient Laboratory Test"
    _order = 'date_analysis desc, id desc'

    @api.model
    def _get_disclaimer(self):
        return self.env.user.sudo().company_id.pb_laboratory_disclaimer or ''

    STATES = {'cancel': [('readonly', True)], 'done': [('readonly', True)]}

    name = fields.Char(string='Test ID', help="Lab result ID", readonly=True,copy=False, index=True, tracking=True)
    test_id = fields.Many2one('pb.lab.test', string='Test', required=True, ondelete='restrict', tracking=True)
    ...
    request_id = fields.Many2one('pb.laboratory.request', string='Test Request', ondelete='restrict')
    laboratory_id = fields.Many2one('pb.laboratory', related="request_id.laboratory_id", string='Laboratory', readonly=True, store=True)
    report = fields.Text(string='Test Report')
    note = fields.Text(string='Extra Info')
    sample_ids = fields.Many2many('pb.patient.laboratory.sample', 'test_lab_sample_rel', 'test_id', 'sample_id', string='Test Samples')
    ...
    collection_center_id = fields.Many2one('pb.laboratory', string='Collection Center', related="request_id.collection_center_id")
    ...
    def _track_log_get_default_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'done':
            return self.env.ref('pb_laboratory.mt_lab_test_done')
        return super(PatientLabTest, self)._track_log_get_default_subtype(init_values)

    @api.depends('name', 'test_id.name')
    ...
    def _subscribe_physician(self):
        done_subtype = self.env.ref('pb_laboratory.mt_lab_test_done').id
    ...
    def action_lab_test_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('pb_laboratory.pb_lab_test_email', raise_if_not_found=False)
