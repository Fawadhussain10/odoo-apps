...
class Accounting_reportPartner_ledger(models.TransientModel):
    ...
    def print_partner_ledger(self):
        ...
        if self._context.get('report_type') != 'excel':
            return self.env.ref('partner_ledger_report.report_partnerledger_action').with_context(
            ...
            for partner in docs:
                ...
                debit = self.env['report.partner_ledger_report.report_partnerledger'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'debit')
                credit = self.env['report.partner_ledger_report.report_partnerledger'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'credit')
                balance = self.env['report.partner_ledger_report.report_partnerledger'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'debit - credit')
                ...
                if self.initial_balance :
                    final_data=self.env['report.partner_ledger_report.report_partnerledger'].with_context(
                    ...
                    if final_data: 
                        for line in final_data:
                            ...
                            row +=1
                for line in self.env['report.partner_ledger_report.report_partnerledger'].with_context(
