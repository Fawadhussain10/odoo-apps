...
class PBQrcodeMixin(models.AbstractModel):
    _name = "pb.qrcode.mixin"
    _description = "QrCode Mixin"

    unique_code = fields.Char("Unique UID")
    qr_image = fields.Binary("QR Code", compute='pb_generate_qrcode')

    def pb_generate_qrcode(self):
...
class PBHmsMixin(models.AbstractModel):
    _name = "pb.hms.mixin"
    _description = "HMS Mixin"

    def pb_prepare_invocie_data(self, partner, patient, product_data, inv_data):
        ...
        data = {
            ...
            'invoice_line_ids': self.pb_get_invoice_lines(product_data, partner, inv_data, fiscal_position_id),
        ...
        commission_installed = bool(self.env['ir.module.module'].sudo().search_count(
            [('name', '=', 'pb_hms_commission'), ('state', '=', 'installed')]))
        if commission_installed and self.env.context.get('commission_partner_ids',False):
            data['commission_partner_ids'] = [(6, 0, [self.env.context.get('commission_partner_ids')])]
        return data

    @api.model
    def pb_create_invoice(self, partner, patient=False, product_data=[], inv_data={}):
        inv_data = self.pb_prepare_invocie_data(partner, patient, product_data, inv_data)
    ...
    @api.model
    def pb_get_invoice_lines(self, product_data, partner, inv_data, fiscal_position_id):
        lines = []
        for data in product_data:
            ...
            if product:
                pb_pricelist_id = self.env.context.get('pb_pricelist_id')

                if not data.get('price_unit'):
                    price = product._pb_get_partner_price(quantity, uom_id, partner)
                else:
    ...
    @api.model
    def pb_create_invoice_line(self, product_data, invoice):
        ...
        if product:
            if not product_data.get('price_unit'):
                price = product._pb_get_partner_price(quantity, uom_id, invoice.partner_id)
            else:
    ...
    def pb_action_view_invoice(self, invoices):
        ...
        elif self.env.context.get('pb_open_blank_list'):
    ...
    def pb_apply_invoice_exemption(self):
        for rec in self:
            rec.invoice_exempt = False if rec.invoice_exempt else True


class PBDocumntMixin(models.AbstractModel):
    _name = "pb.documnt.mixin"
    ...
    def _pb_attachemnt_count(self):
        for rec in self:
            attachments = rec._pb_get_attachemnts()
            rec.attach_count = len(attachments)
            rec.attachment_ids = [(6,0,attachments.ids)]

    attach_count = fields.Integer(compute="_pb_attachemnt_count", readonly=True, string="Documents")
    attachment_ids = fields.Many2many('ir.attachment', 'attachment_pb_hms_rel', 'record_id', 'attachment_id', compute="_pb_attachemnt_count", string="Attachments")
...
class PBAppointmentConsumable(models.Model):
    _name = "hms.consumable.line"
    _description = "List of Consumables"

    @api.depends('price_unit','qty')
    def pb_get_total_price(self):
        for rec in self:
    ...
    product_uom_id = fields.Many2one('uom.uom', string='Unit of Measure', help='Amount of medication (eg, 250 mg) per dose', domain="['|', ('id', '=', product_uom_category_id), ('relative_uom_id', '=', product_uom_category_id)]")
    ...
    subtotal = fields.Float(compute=pb_get_total_price, string='Subtotal', readonly=True, store=True)
...
class PBPatientTag(models.Model):
    _name = "hms.patient.tag"
    _description = "Pb Patient Tag"
...
class PBTherapeuticEffect(models.Model):
    _name = "hms.therapeutic.effect"
    _description = "Pb Therapeutic Effect"
