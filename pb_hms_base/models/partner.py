...
class ResPartner(models.Model):
    ...
    # inverse of hms.patient.partner_id, used by the ir.access domain letting
    # HMS users update the contact records behind patients
    pb_hms_patient_ids = fields.One2many('hms.patient', 'partner_id', string='Patients')
    ...
    pb_amount_due = fields.Monetary(compute='_compute_pb_amount_due',currency_field='currency_id')
    pb_patient_id = fields.Many2one('hms.patient', compute='_is_patient', string='Patient', readonly=True)

    def _compute_pb_amount_due(self):
