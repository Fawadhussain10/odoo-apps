...
class PbHospitalizationForecast(models.TransientModel):
    _name = "pb.hospitalization.forecast"
    ...
    def get_pb_final_due(self):
        for rec in self:
            rec.pb_final_due = rec.pb_amount_due + rec.amount_total

    hospitalization_id = fields.Many2one("pb.hospitalization", string="Hospitalization", readonly=True, required=True)
    ...
    line_ids = fields.One2many("pb.hospitalization.forecast.line", 'order_id', string='Forecast Lines', readonly=False, copy=True)
    ...
    pb_amount_due = fields.Monetary(related="patient_id.pb_amount_due", string="Due Amount")
    pb_final_due = fields.Monetary(compute=get_pb_final_due, string="Total Due Amount")


    @api.onchange('hospitalization_id')
    def onchange_hospitalization(self):
        ForecastLine = self.env['pb.hospitalization.forecast.line']
        if self.hospitalization_id:
            ...
            lines = []
            data = hospitalization.pb_hospitalization_invoicing()            
            for product_data in data:
                product = product_data.get('product_id')

                if product:
                    pb_pricelist_id = self.env.context.get('pb_pricelist_id')
                    if not product_data.get('price_unit') and (self.partner_id.property_product_pricelist or pb_pricelist_id):
                        if pb_pricelist_id:
                            pricelist_id = self.env['product.pricelist'].browse(pb_pricelist_id)
                        else:
...
class PbHospitalizationForecastLine(models.TransientModel):
    _name = 'pb.hospitalization.forecast.line'
    ...
    order_id = fields.Many2one('pb.hospitalization.forecast', string='Order', required=True, ondelete='cascade', index=True, copy=False)
