...
class Hospitalization(models.Model):
    ...
    evaluation_ids = fields.One2many('pb.patient.evaluation', 'hospitalization_id', '#Evaluations')
    ...
    last_evaluation_id = fields.Many2one("pb.patient.evaluation", related='patient_id.last_evaluation_id', string="Last Evaluation")
    ...
    pb_weight_name = fields.Char(related="last_evaluation_id.pb_weight_name", string='Patient Weight unit of measure label')
    pb_height_name = fields.Char(related="last_evaluation_id.pb_height_name", string='Patient Height unit of measure label')
    pb_temp_name = fields.Char(related="last_evaluation_id.pb_temp_name", string='Patient Temp unit of measure label')
    pb_spo2_name = fields.Char(related="last_evaluation_id.pb_spo2_name", string='Patient SpO2 unit of measure label')
    pb_rbs_name = fields.Char(related="last_evaluation_id.pb_rbs_name", string='Patient RBS unit of measure label')
    ...
    def action_view_evaluation(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.action_pb_patient_evaluation")
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            if values.get('name', 'Hospitalization#') == 'Hospitalization#':
                values['name'] = self.env['ir.sequence'].next_by_code('pb.hospitalization') or 'Hospitalization#'
    ...
    def action_accommodation_history(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_hospitalization.action_accommodation_history")
        action['domain'] = [('hospitalization_id', '=', self.id)]
        action['context'] = {
            'default_patient_id': self.patient_id.id,
            'default_hospitalization_id': self.id}
    ...
    def action_view_surgery(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_surgery.action_hms_surgery")
        action['domain'] = [('hospitalization_id', '=', self.id)]
        action['context'] = {
            'default_patient_id': self.patient_id.id,
            'default_hospitalization_id': self.id}
    ...
    def pb_get_consume_locations(self):
        if not self.company_id.pb_hospitalization_usage_location_id:
            raise UserError(_('Please define a location where the consumables will be used during the surgery in company.'))
        if not self.company_id.pb_hospitalization_stock_location_id:
            raise UserError(_('Please define a hospitalization location from where the consumables will be taken.'))

        dest_location_id  = self.company_id.pb_hospitalization_usage_location_id.id
        source_location_id  = self.company_id.pb_hospitalization_stock_location_id.id
    ...
    def pb_hospitalization_physician_round_data(self, invoice_id=False):
        product_data = []
    ...
    def pb_hospitalization_prescription_data(self, invoice_id=False):
        pres_data = []
        ...
        pharmacy_installed = bool(self.env['ir.module.module'].sudo().search_count(
            [('name', '=', 'pb_hms_pharmacy'), ('state', '=', 'installed')]))
        if pharmacy_installed and prescription_ids:
    ...
    def pb_hospitalization_lab_data(self, invoice_id=False):
        return []

    #In radio module it get implemented.
    def pb_hospitalization_radiology_data(self, invoice_id=False):
        return []

    #In nursing module it get implemented.
    def pb_hospitalization_nurse_round_data(self, invoice_id=False):
        return []

    #Keep Sub methods for projection flow. Because it just return list of data. Do not create real invoice.
    def pb_hospitalization_invoicing(self, invoice_id=False):
        ...
        #consumable Invoicing
        physician_round_data = self.pb_hospitalization_physician_round_data(invoice_id)

        #Nurse Round Invoicing
        nurse_round_data = self.pb_hospitalization_nurse_round_data(invoice_id)

        #surgey Invoicing
        surgery_data = self.get_surgery_invoice_data(invoice_id)
        
        #Pharmacy Invoicing
        pres_data = self.pb_hospitalization_prescription_data(invoice_id)

        #Lab Invoicing
        lab_data = self.pb_hospitalization_lab_data(invoice_id)

        #Radiology Invoicing
        radiology_data = self.pb_hospitalization_radiology_data(invoice_id)
        ...
        if invoice_id:
            for line in data:
                ...
                inv_line = self.with_context(pb_pricelist_id=pricelist_id).pb_create_invoice_line(line, invoice_id)
                #PB: As on accomodation history we need to set inv line as special case it is managed here.
    ...
    def action_create_invoice(self):
        ...
        pb_context = {'commission_partner_ids':self.physician_id.partner_id.id}
        invoice_id = self.with_context(pb_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
        invoice_id.hospitalization_id = self.id

        self.pb_hospitalization_invoicing(invoice_id)
    ...
    def pb_invoice_forecast(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_hospitalization.action_pb_hospitalization_forecast")
        action['domain'] = [('hospitalization_id', '=', self.id)]
        rec_id = self.env['pb.hospitalization.forecast'].create({
