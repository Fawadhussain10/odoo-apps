...
class AdmissionCheckList(models.Model):
    ...
    remark = fields.Char(string="Remarks")
    hospitalization_id = fields.Many2one("pb.hospitalization", ondelete="cascade", string="Hospitalization")
...
class PatientAccommodationHistory(models.Model):
    ...
    def pb_get_move_lines_data(self):
        for rec in self:
            rec.invoiced_rest_time = sum(rec.account_move_line_ids.mapped('quantity'))

    hospitalization_id = fields.Many2one('pb.hospitalization', ondelete="cascade", string='Inpatient')
    ...
    account_move_line_ids = fields.Many2many('account.move.line', 'pb_accomodation_account_move_line_rel', 'move_id', 'accommodation_id', string='Invoice Lines')
    invoiced_rest_time = fields.Float(compute="pb_get_move_lines_data", string="Invoiced Rest Time")
...
class PBCarePlanTemplate(models.Model):
