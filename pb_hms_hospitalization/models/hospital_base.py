...
class PBHospitalWard(models.Model):
    _name = 'hospital.ward'
    ...
    def action_bed(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_hospitalization.action_bed")
    ...
    def copy(self, default=None):
        ...
        new_name = chosen_name or _('%s (copy)') % self.name
        default = dict(default or {}, name=new_name)
        return super(PBHospitalWard, self).copy(default)


class PBHospitalBuilding(models.Model):
...
class PBHospitalOT(models.Model):
    _name = 'pb.hospital.ot'
