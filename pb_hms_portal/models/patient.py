...
class Patient(models.Model):
    ...
    def pb_get_family_partners(self):
        for rec in self:
            family_partner_ids = rec.inverse_family_member_ids.mapped('patient_id.partner_id').ids
            rec.pb_family_partner_ids = [(6,0,family_partner_ids)]

    def _pb_get_patient_portal_line_graph(self):
        for rec in self:
            records = self.env['pb.patient.evaluation'].search([('patient_id','=',rec.id)], order="date", limit=100)
            labels = []
            weight_data = []
            height_data = []
            temp_data = []
    ...
    inverse_family_member_ids = fields.One2many('pb.family.member', 'related_patient_id', string='Related Family')
    pb_family_partner_ids = fields.Many2many('res.partner', compute="pb_get_family_partners", store=True, string="Family Partners")
    patient_portal_line_graph = fields.Text(compute='_pb_get_patient_portal_line_graph')
    ...
    def create_patient_related_user(self):
        for record in self:
            ...
            #PB NOTE: incase of sigup from website it takes portal user. 
    ...
    def send_invitaion_email(self):
        for record in self:
            ...
            template = self.env.ref('pb_hms_portal.set_password_email')
