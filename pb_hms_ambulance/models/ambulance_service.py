...
class PbAmbulanceService(models.Model):
    _name = 'pb.ambulance.service'
    ...
    def _get_service_id(self):
        ...
        if self.env.user.company_id.pb_ambulance_invoicing:
            service_product = self.env.user.company_id.pb_ambulance_invoicing_product_id.id
    ...
    pb_ambulance_invoicing = fields.Boolean(related='company_id.pb_ambulance_invoicing',string='Allow Ambulance Service Invoicing', readonly=True)
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('name', '/') == '/':
                values['name'] = self.env['ir.sequence'].next_by_code('pb.ambulance.service') or ''
    ...
    def unlink(self):
        ...
        return super(PbAmbulanceService, self).unlink()
    ...
    def action_done(self):
        ...
        if self.env.user.company_id.pb_ambulance_invoicing and not self.invoice_exempt:
    ...
    def action_create_invoice(self):
        product_id = self.service_product_id or self.env.user.company_id.pb_ambulance_invoicing_product_id
        if not product_id:
