...
class WhatsappAccount(models.Model):
    ...
    def _notify_users(self, chat, message):
        ...
        # Odoo 18 subscribes each user's browser to the groups the user belongs to (implied
        # groups included), so one message reaches exactly the members of the chat group
        group = self.env.ref('whatsapp_qr_connect.group_whatsapp_chat')
        self.env['bus.bus'].sudo()._sendone(group, 'whatsapp_qr_connect.new_message', payload)
