...
class TestWhatsappQrConnect(TransactionCase):
    ...
    def test_normalize_phone(self):
        ...
        self.env['ir.config_parameter'].set_str('whatsapp_qr_connect.default_country_code', '92')
    ...
    def test_non_admin_cannot_manage_but_internal_can_send(self):
        user = self.env['res.users'].create({
            'name': 'Plain User', 'login': 'plain_wa', 'group_ids': [(6, 0, [self.env.ref('base.group_user').id])],
        })
...
class TestWorkerPython(TransactionCase):
    ...
    def test_interpreter_without_neonize_is_reported(self):
        ...
        self.env['ir.config_parameter'].set_str('whatsapp_qr_connect.python_path', '/bin/false')
