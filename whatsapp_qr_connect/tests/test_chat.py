...
class TestWhatsappChat(TransactionCase):
    ...
    def test_only_the_chat_group_can_use_the_screen(self):
        ...
        plain = Users.create({'name': 'I', 'login': 'wa_plain', 'groups_id': [(6, 0, [internal.id])]})
        member = Users.create({'name': 'M', 'login': 'wa_member',
                               'groups_id': [(6, 0, [internal.id, chat_group.id])]})
        with self.assertRaises(AccessError):
        ...
        self.assertEqual(menu.groups_id, chat_group)
        ...
        # Odoo 18 subscribes each browser to its groups: one push to the chat group reaches only its members
        self.env['bus.bus'].sudo().search([]).unlink()
        self.account._process_incoming([self._incoming('GRP1')])
        self.env.cr.precommit.run()
        targets = ' '.join(self.env['bus.bus'].sudo().search([]).mapped('channel'))
        self.assertIn('"res.groups",%d]' % chat_group.id, targets)
        self.assertIn(chat_group, member.groups_id)
        self.assertNotIn(chat_group, plain.groups_id)
