...
class ResUsers(models.Model):
    ...
    birthday_color = fields.Char(string='Birthday Color', default="#059669", user_writeable=True)

    #Receptionist Dashboard fields
    total_patients = fields.Integer(compute="_compute_dashboard_data")
    total_patients_color = fields.Char(string='Total Patient Color', default="#0D9488", user_writeable=True)
    ...
    total_treatments_color = fields.Char(string='Total Treatments Color', default="#059669", user_writeable=True)

    total_appointments = fields.Integer(compute="_compute_dashboard_data")
    total_appointments_color = fields.Char(string='Total Appointments Color', default="#D97706", user_writeable=True)
    ...
    total_open_invoice_color = fields.Char(string='Open Invoices', default="#E11D48", user_writeable=True)

    total_shedules = fields.Integer(compute="_compute_dashboard_data")
    total_shedules_color = fields.Char(string='Total Shedules Color', default="#4F46E5", user_writeable=True)

    appointment_bar_graph = fields.Text(compute='_compute_dashboard_data')
    appointment_bar_graph_color = fields.Char(string='Patient Barchart Color', default="#0D9488", user_writeable=True)
    patient_line_graph = fields.Text(compute='_compute_dashboard_data')
    patient_line_graph_color = fields.Char(string='Patient Linechart Color', default="#4F46E5", user_writeable=True)
    ...
    my_total_patients_color = fields.Char(string='My Total Patient Color', default="#0D9488", user_writeable=True)
    my_total_appointments = fields.Integer(compute="_compute_dashboard_data")
    my_total_appointments_color = fields.Char(string='My Total Appointments Color', default="#D97706", user_writeable=True)
    my_avg_wait_time = fields.Char(compute="_compute_dashboard_data")
    my_avg_cons_time = fields.Char(compute="_compute_dashboard_data")
    my_avg_time_color = fields.Char(string='My Avg Time Color', default="#4F46E5", user_writeable=True)
    ...
    my_total_treatments_color = fields.Char(string='My Total Treatments Color', default="#059669", user_writeable=True)

    #Admin fields
    avg_wait_time = fields.Char(compute="_compute_dashboard_data")
    avg_cons_time = fields.Char(compute="_compute_dashboard_data")
    avg_time_color = fields.Char( string='Avg Time Color', default="#4F46E5", user_writeable=True)
    ...
    physicians_color = fields.Char(string='Total Physicians Color', default="#0891B2", user_writeable=True)

    def _get_user_role(self):
        for rec in self:
            rec.is_physician = True if (rec.physician_count > 0) else False
            rec.is_manager = rec.has_group('pb_hms_base.group_hms_manager')
        ...
        for i in range(-1,4):
            if i==0:
                ...
                if start_week.month == end_week.month:
                    label = str(start_week.day) + '-' +str(end_week.day)+ ' ' + format_date(end_week, 'MMM', locale=self.env.context.get('lang') or 'en_US')
                else:
                    label = format_date(start_week, 'd MMM', locale=self.env.context.get('lang') or 'en_US')+'-'+format_date(end_week, 'd MMM', locale=self.env.context.get('lang') or 'en_US')
    ...
    #PB: t-att-name was not supported so passed method name in context and called it.
    def pb_open_dashboard_action(self):
        method = self.env.context.get('pb_action')
        if not method:
    ...
    def open_patients(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_base.action_patient")
    ...
    def open_referring_physicians(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.action_referring_doctors")
