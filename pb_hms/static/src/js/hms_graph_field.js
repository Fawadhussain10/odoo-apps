...
export class PbHmsGraphField extends Component {
    ...
    chart = useChart(() => this.getLineChartConfig());

    hasData() {
        const data = this.parseValue(this.props.record.data[this.props.name]);
        return Boolean(data.length && data[0] && data[0].values && data[0].values.length);
    }
    ...
    getLineChartConfig() {
        this.data = this.parseValue(this.props.record.data[this.props.name]);
        if (!this.hasData()) {
