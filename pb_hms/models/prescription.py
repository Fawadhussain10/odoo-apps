...
class PBPrescriptionOrder(models.Model):
    _name='prescription.order'
    _description = "Prescription Order"
    _inherit = ['mail.thread', 'mail.activity.mixin', 'pb.hms.mixin', 'pb.qrcode.mixin']
    _order = 'id desc'
    ...
    patient_id = fields.Many2one('hms.patient', ondelete="restrict", string='Patient', required=True, tracking=True)
    ...
    prescription_date = fields.Datetime(string='Prescription Date', required=True, default=fields.Datetime.now, tracking=True, copy=False)
    physician_id = fields.Many2one('hms.physician', ondelete="restrict", string='Prescribing Doctor', default=_current_user_doctor, tracking=True)
    ...
    medical_alert_ids = fields.Many2many('pb.medical.alert', 'prescription_medical_alert_rel','prescription_id', 'alert_id',
    ...
    def unlink(self):
        ...
        return super(PBPrescriptionOrder, self).unlink()
    ...
    def print_report(self):
        return self.env.ref('pb_hms.report_hms_prescription_id').report_action(self)
    ...
    def action_prescription_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('pb_hms.pb_prescription_email', raise_if_not_found=False)
        ctx = {
...
class PBPrescriptionLine(models.Model):
    ...
    dosage_uom_id = fields.Many2one('uom.uom', string='Unit of Dosage', help='Amount of Medicine (eg, mg) per dose', domain="['|', ('id', '=', product_uom_category_id), ('relative_uom_id', '=', product_uom_category_id)]")
