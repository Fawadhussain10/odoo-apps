...
class ResUsers(models.Model):
    ...
    department_ids = fields.Many2many('hr.department', 'user_department_rel', 'user_id','department_id', 
        domain=[('patient_department', '=', True)], string='Departments', user_writeable=True)
    physician_count = fields.Integer(string="#Physician", compute="_compute_physician_count")
    physician_ids = fields.One2many('hms.physician', 'user_id', string='Related Physician', user_writeable=True)
...
class PBEthnicity(models.Model):
...
class PBMedicalAlert(models.Model):
    _name = 'pb.medical.alert'
...
class PBFamilyRelation(models.Model):
    _name = 'pb.family.relation'
    _description = "Family Relation"
    _order = "sequence"

    name = fields.Char(required=True)
    sequence = fields.Integer(string='Sequence', default=10)
    inverse_relation_id = fields.Many2one("pb.family.relation", string="Inverse Relation")

    @api.depends('name', 'inverse_relation_id.name')
    ...
    def write(self, values):
        res = super(PBFamilyRelation, self).write(values)
...
class PBConsumableLine(models.Model):
    ...
    #PB: In case of kit moves set move_ids but add move_id also. Else it may lead to comume material process again.
...
class Physician(models.Model):
    ...
    def create(self, vals_list):
        res = super().create(vals_list)
        for record in res:
            record.group_ids = [(4, self.env.ref('pb_hms.group_hms_jr_doctor').id)]
        return res
