...
class PBTreatment(models.Model):
    _name = 'hms.treatment'
    ...
    patient_id = fields.Many2one('hms.patient', 'Patient', required=True, index=True, tracking=True)
    department_id = fields.Many2one('hr.department', ondelete='restrict', string='Department',
        domain=[('patient_department', '=', True)], tracking=True)
    ...
    physician_id = fields.Many2one('hms.physician', ondelete='restrict', string='Physician',
        help='Physician who treated or diagnosed the patient', tracking=True)
    ...
    medical_alert_ids = fields.Many2many('pb.medical.alert', 'treatment_medical_alert_rel','treatment_id', 'alert_id',
    ...
    patient_procedure_ids = fields.One2many('pb.patient.procedure', 'treatment_id', 'Patient Procedures')
    ...
    def default_get(self, fields):
        res = super(PBTreatment, self).default_get(fields)
    ...
    def unlink(self):
        ...
        return super(PBTreatment, self).unlink()
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(pb_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'treatment'})
