...
class PbPatientProcedure(models.Model):
    ...
    def _pb_get_attachemnts(self):
        attachments = super(PbPatientProcedure, self)._pb_get_attachemnts()
    ...
    duration = fields.Float('Duration', compute="pb_get_duration", store=True)

    diseas_id = fields.Many2one('hms.diseases', 'Disease')
    description = fields.Text(string="Description")
    treatment_id = fields.Many2one('hms.treatment', 'Treatment')
    appointment_ids = fields.Many2many('hms.appointment', 'pb_appointment_procedure_rel', 'appointment_id', 'procedure_id', 'Appointments')
    ...
    def default_get(self, fields):
        res = super(PbPatientProcedure, self).default_get(fields)
    ...
    def unlink(self):
        ...
        return super(PbPatientProcedure, self).unlink()

    @api.model_create_multi
    def create(self, vals_list):
        for values in vals_list:
            values['name'] = self.env['ir.sequence'].next_by_code('pb.patient.procedure') or 'New Procedure'
        return super().create(vals_list)

    def pb_patient_procedure_inv_product_data(self):
        product_data = []
    ...
    def action_create_invoice(self):
        product_data = self.pb_patient_procedure_inv_product_data()
    ...
    def action_show_details(self):
        ...
        action['views'] = [(self.env.ref('pb_hms.view_pb_patient_procedure_form').id, 'form')]
        action['target'] = 'new'
        return action
...
class StockMove(models.Model):
    ...
    procedure_id = fields.Many2one('pb.patient.procedure', ondelete="cascade", string="Procedure")
