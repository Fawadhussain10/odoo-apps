...
class PbCancelReason(models.Model):
    _name = 'pb.cancel.reason'
...
class Appointment(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'pb.hms.mixin', 'pb.documnt.mixin']
    _order = "id desc"
    ...
    def _pb_get_invoice_count(self):
        AccountMove = self.env['account.move']
        for rec in self:
            rec.invoice_count = len(rec.invoice_ids)

    def _pb_invoice_policy(self):
        for rec in self:
    ...
    def get_procudures_to_invoice(self):
        Procedure = self.env['pb.patient.procedure']
        for rec in self:
            procedures = Procedure.search([('appointment_ids', 'in', rec.id), ('invoice_id','=', False)])
            rec.procedure_to_invoice_ids = [(6, 0, procedures.ids)]

    def pb_get_department(self):
        for rec in self:
            pb_department_id = False
            if rec.department_id and rec.department_id.id:
                pb_department_id = self.env['hr.department'].sudo().search([('id','=',rec.department_id.id)]).id
            rec.pb_department_id = pb_department_id
    ...
    name = fields.Char(string='Appointment Id', copy=False, tracking=True)
    ...
    #PB: Added department field agian here to avoid portal error. Insted of reading department_id used pb_department_idfield so error vanbe avoided.
    pb_department_id = fields.Many2one('hr.department', compute="pb_get_department")
    ...
    pb_reminder_sent = fields.Boolean("Reminder Sent", default=False)

    evaluation_ids = fields.One2many('pb.patient.evaluation', 'appointment_id', 'Evaluations')
    evaluation_id = fields.Many2one('pb.patient.evaluation', ondelete='restrict', compute=_get_evaluation,
    ...
    pb_weight_name = fields.Char(related="evaluation_id.pb_weight_name", string='Patient Weight unit of measure label')
    pb_height_name = fields.Char(related="evaluation_id.pb_height_name", string='Patient Height unit of measure label')
    pb_temp_name = fields.Char(related="evaluation_id.pb_temp_name", string='Patient Temp unit of measure label')
    pb_spo2_name = fields.Char(related="evaluation_id.pb_spo2_name", string='Patient SpO2 unit of measure label')
    pb_rbs_name = fields.Char(related="evaluation_id.pb_rbs_name", string='Patient RBS unit of measure label')
    ...
    appointment_invoice_policy = fields.Selection([('at_end','Invoice in the End'),
        ...
        ('advance','Invoice in Advance')],compute=_pb_invoice_policy, string="Appointment Invoicing Policy")
    ...
    date_to = fields.Datetime(string='Date To', default=fields.Datetime.now() + timedelta(minutes=15), copy=False, tracking=True)
    planned_duration = fields.Float('Duration', compute="_get_planned_duration", inverse='_inverse_planned_duration',
        default=lambda self: self.env.company.pb_appointment_planned_duration)
    ...
    treatment_id = fields.Many2one('hms.treatment', ondelete='cascade', 
        string='Treatment', help="Treatment Id", tracking=True)
    ...
    medical_alert_ids = fields.Many2many('pb.medical.alert', 'appointment_medical_alert_rel','appointment_id', 'alert_id',
    ...
    cancel_reason_id = fields.Many2one('pb.cancel.reason', string='Cancellation Reason')
    user_id = fields.Many2one('res.users',string='Responsible',
        ondelete='cascade', help='Responsible User for appointment validation And further Followup.')
    pb_kit_id = fields.Many2one('pb.product.kit', string='Kit')
    pb_kit_qty = fields.Integer("Kit Qty", default=1)
    invoice_ids = fields.One2many("account.move", "appointment_id", string="Invoices", groups="account.group_account_invoice")
    invoice_count = fields.Integer(compute="_pb_get_invoice_count", string="#Invoices", groups="account.group_account_invoice")
    procedure_to_invoice_ids = fields.Many2many('pb.patient.procedure', 'pb_appointment_procedure_rel', 'appointment_id', 'procedure_id', compute="get_procudures_to_invoice", string="Procedures to Invoice")
    ...
    refered_from_physician_id = fields.Many2one('hms.physician', related='refered_from_appointment_id.physician_id', string='Refered from Physician', tracking=True, store=True)
    refered_from_reason = fields.Text(related='refered_from_appointment_id.refer_reason', string='Refered From Reason', tracking=True, store=True)

    refered_to_appointment_id = fields.Many2one("hms.appointment", string="Refered Appointment")
    refered_to_physician_id = fields.Many2one('hms.physician', related='refered_to_appointment_id.physician_id', ondelete='restrict', string='Refered to Physician', tracking=True, store=True)

    #PB NOTE: Because of error for portal appointment creation added _compute_field_value method.
    ...
    pb_show_in_wc = fields.Boolean(default=True)
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get('pb_department_type'):
            department = self.env['hr.department'].search([('department_type','=',self.env.context.get('pb_department_type'))], limit=1)
            if department:
    ...
    def update_reminder_dates(self):
        for rec in self:
            if fields.Datetime.now() < rec.date:
                reminder_date = rec.date - timedelta(days=int(rec.company_id.pb_reminder_day)) - timedelta(hours=int(rec.company_id.pb_reminder_hours))
    ...
    def send_appointment_reminder(self):
        ...
        reminder_appointments = self.sudo().search([('pb_reminder_sent', '=', False),('state', 'in', ['draft','confirm']),('date','>',fields.Datetime.now()),('reminder_date','<=', date_time_now)])
        if reminder_appointments:
            for reminder_appointment in reminder_appointments:
                reminder_appointment.pb_reminder_sent = True
    ...
    def print_report(self):
        return self.env.ref('pb_hms.action_appointment_report').report_action(self)
    ...
    def action_appointment_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('pb_hms.pb_appointment_email', raise_if_not_found=False)
    ...
    def pb_appointment_inv_product_data(self, with_product=True):
        product_data = []
        if with_product:
    ...
    def pb_appointment_inv_data(self):
        return {
    ...
    def create_invoice(self):
        inv_data = self.pb_appointment_inv_data()
        product_data = self.pb_appointment_inv_product_data()
        pb_context = {'commission_partner_id':self.physician_id.partner_id.id}
        if self.pricelist_id:
            pb_context.update({'pb_pricelist_id': self.pricelist_id.id})
        invoice = self.with_context(pb_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
        self.invoice_id = invoice.id
        if self.state == 'to_invoice':
            self.appointment_done()

        if self.state == 'draft' and not self.env.context.get('avoid_confirmation'):
            if self.invoice_id and not self.company_id.pb_check_appo_payment:
    ...
    def create_consumed_prod_invoice(self):
        ...
        inv_data = self.pb_appointment_inv_data()
        product_data = self.pb_appointment_inv_product_data(with_product=False)

        pricelist_context = {}
        if self.pricelist_id:
            pricelist_context = {'pb_pricelist_id': self.pricelist_id.id}
        invoice = self.with_context(pricelist_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def view_invoice(self):
        ...
        action = self.pb_action_view_invoice(appointment_invoices)
    ...
    def action_refer_doctor(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.action_appointment")
        ...
        action['views'] = [(self.env.ref('pb_hms.view_hms_appointment_form').id, 'form')]
        return action

    def action_create_evaluation(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.action_pb_patient_evaluation_popup")
        action['domain'] = [('patient_id','=',self.id)]
    ...
    def onchange_physician(self):
        product_id = False
        #PB: First check configuration on department.
        if self.pb_department_id:
            #PB: To avoid portal access error research department here.
            if self.consultation_type=='followup':
                if self.pb_department_id.followup_service_id:
                    product_id = self.pb_department_id.followup_service_id.id

            elif self.pb_department_id.consultaion_service_id:
                product_id = self.pb_department_id.consultaion_service_id.id
        ...
        if self.physician_id:
            ...
            if self.physician_id.appointment_duration and not self.env.context.get('pb_online_transaction'):
    ...
    def appointment_confirm(self):
        if not self.env.context.get('pb_online_transaction'):
            if self.appointment_invoice_policy=='advance' and not self.invoice_id:
                raise UserError(_('Invoice is not created yet'))

            elif self.invoice_id and self.company_id.pb_check_appo_payment and self.payment_state not in ['in_payment','paid']:
                raise UserError(_('Invoice is not Paid yet.')) 

        if not self.user_id:
            self.user_id = self.env.user.id

        if self.patient_id.email and (self.company_id.pb_auto_appo_confirmation_mail or self.env.context.get('pb_online_transaction')):
            template = self.env.ref('pb_hms.pb_appointment_email')
    ...
    def action_prescription(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.act_open_hms_prescription_order_view")
    ...
    def button_pres_req(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.act_open_hms_prescription_order_view")        
        action['domain'] = [('appointment_id', '=', self.id)]
        action['views'] = [(self.env.ref('pb_hms.view_hms_prescription_order_form').id, 'form')]
        action['context'] = {
                'default_patient_id': self.patient_id.id,
                'default_physician_id':self.physician_id.id,
                'default_diseases_ids': [(6,0,self.diseases_ids.ids)],
    ...
    def pb_get_consume_locations(self):
    ...
    def consume_appointment_material(self):
        for rec in self:
            source_location_id, dest_location_id = rec.pb_get_consume_locations()
            for line in rec.consumable_line_ids.filtered(lambda s: not s.move_id):
                if line.product_id.is_kit_product:
                    move_ids = []
                    for kit_line in line.product_id.pb_kit_line_ids:
    ...
    def action_view_patient_procedures(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.action_pb_patient_procedure")
    ...
    def get_pb_kit_lines(self):
        if not self.pb_kit_id:
            raise UserError("Please Select Kit first.")

        lines = []
        for line in self.pb_kit_id.pb_kit_line_ids:
            lines.append((0,0,{
                'product_id': line.product_id.id,
                'product_uom_id': line.product_id.uom_id.id,
                'qty': line.product_qty * self.pb_kit_qty,
            }))
    ...
    def _create_edit_followup_reminder(self):
        ...
        for rec in self:
            if rec.follow_date:
                ...
                activity_type = company.pb_followup_activity_type_id or default_activity_type
    ...
    def cancel_old_appointments(self):
        ...
        if parameter.sudo().get_param('pb_hms.cancel_old_appointment'):
