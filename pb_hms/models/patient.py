...
class PBPatient(models.Model):
    ...
    def _pb_get_attachemnts(self):
        attachments = super(PBPatient, self)._pb_get_attachemnts()
    ...
    def _patient_evaluation_graph_data(self):
        for rec in self:
            ...
            records = self.env['pb.patient.evaluation'].search(domain, order="date")
    ...
    def pb_check_cancellation_flag(self):
        pb_flag_days = self.env.user.sudo().company_id.pb_flag_days or 365
        pb_flag_count_limit = self.env.user.sudo().company_id.pb_flag_count_limit or 1
        date_start = fields.Datetime.now() - relativedelta(days=pb_flag_days)
        ...
        for rec in self:
            ...
            if cancelled_appointments >= pb_flag_count_limit:
                show_cancellation_warning_flag = True
            rec.show_cancellation_warning_flag = show_cancellation_warning_flag
            rec.pb_flag_days = pb_flag_days
            rec.pb_cancelled_appointments = cancelled_appointments
    ...
    ethnic_group_id = fields.Many2one('pb.ethnicity', string='Ethnic group')
    cod_id = fields.Many2one('hms.diseases', string='Cause of Death')
    family_member_ids = fields.One2many('pb.family.member', 'patient_id', string='Family')
    ...
    medical_alert_ids = fields.Many2many('pb.medical.alert', 'patient_medical_alert_rel','patient_id', 'alert_id',
    ...
    evaluation_ids = fields.One2many('pb.patient.evaluation', 'patient_id', 'Evaluations')

    last_evaluation_id = fields.Many2one("pb.patient.evaluation", string="Last Evaluation", compute=_get_last_evaluation, readonly=True, store=True)
    ...
    patient_procedure_ids = fields.One2many('pb.patient.procedure', 'patient_id', 'Patient Procedures')
    patient_procedure_count = fields.Integer(compute='_rec_count', string='# Patient Procedures')
    show_cancellation_warning_flag = fields.Boolean(compute='pb_check_cancellation_flag', string='Show Cancellation Flag')
    pb_flag_days = fields.Integer(compute='pb_check_cancellation_flag', string='Flag Days')
    pb_cancelled_appointments = fields.Integer(compute='pb_check_cancellation_flag', string='Cancelled Appointments')
    ...
    def show_weight_chart(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.action_patient_evaluation_graph_1")
    ...
    def create_invoice(self):
        ...
        invoice = self.pb_create_invoice(partner=self.partner_id, patient=self, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'patient'})
    ...
    def action_treatment(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms.pb_action_form_hospital_treatment")
        action['domain'] = [('patient_id','=',self.id)]
        action['context'] = {'default_patient_id': self.id, 'default_physician_id': self.primary_physician_id.id}
        return action
...
class PBFamilyMember(models.Model):
    _name = 'pb.family.member'
    ...
    relation_id = fields.Many2one('pb.family.relation', string='Relation', required=True)
    inverse_relation_id = fields.Many2one("pb.family.member", string="Inverse Relation")
    ...
    def unlink(self):
        ...
        res = super(PBFamilyMember, self).unlink()
        if inverse_relation_id:
            inverse_relation_id.unlink()
        return res

    def write(self, values):
        res = super(PBFamilyMember, self).write(values)
