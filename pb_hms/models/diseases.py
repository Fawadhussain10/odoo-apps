...
class PBDiseases(models.Model):
    ...
    def _compute_display_name(self):
        for rec in self:
            if rec.code:
                name = (rec.name or '') + ' ' + '['+ str(rec.code) + ']'
            else:
                name = rec.name
            rec.display_name = name

class PBDiseasesCategory(models.Model):
...
class PBPatientDisease(models.Model):
...
class PBDiseaseGene(models.Model):
    _name = 'disease.gene'
