...
class PbBloodRequisition(models.Model):
    _name = 'pb.blood.requisition'
    ...
    def _get_service_id(self):
        ...
        if self.env.user.company_id.pb_blood_requisition_product_id:
            service_product = self.env.user.company_id.pb_blood_requisition_product_id.id
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('name', '/') == '/':
                values['name'] = self.env['ir.sequence'].next_by_code('pb.blood.requisition') or ''
    ...
    def unlink(self):
        ...
        return super(PbBloodRequisition, self).unlink()

    def action_running(self):
        self.state = 'running'

    def action_done(self):
        if self.env.user.company_id.pb_blood_requisition_invoicing:
    ...
    def action_create_invoice(self):
        product_id = self.service_product_id or self.env.user.company_id.pb_blood_requisition_product_id
        if not product_id:
        ...
        invoice = self.pb_create_invoice(partner=self.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def view_invoice(self):
        ...
        return self.pb_action_view_invoice(invoices)


class PbBloodIssuance(models.Model):
    _name = 'pb.blood.issuance'
    ...
    def _get_service_id(self):
        service_product = False
        if self.env.user.company_id.pb_blood_issuance_product_id:
            service_product = self.env.user.company_id.pb_blood_issuance_product_id.id
        return service_product
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('name', '/') == '/':
                values['name'] = self.env['ir.sequence'].next_by_code('pb.blood.issuance') or ''
        return super().create(vals_list)
    ...
    def unlink(self):
        for data in self:
            if data.state in ['done']:
                raise UserError(('You can not delete record in done state'))
        return super(PbBloodIssuance, self).unlink()

    def pb_get_consume_locations(self):
        if not self.company_id.pb_blood_stock_location_id:
            raise UserError(_('Please define a Blood Bank Stock Usage Location where the consumables will be used.'))
        if not self.company_id.pb_blood_usage_location_id:
            raise UserError(_('Please define a Blood Bank Stock Location from where the product will be taken.'))

        dest_location_id  = self.company_id.pb_blood_usage_location_id.id
        source_location_id  = self.company_id.pb_blood_stock_location_id.id
    ...
    def action_create_invoice(self):
        service_product_id = self.service_product_id or self.env.user.company_id.pb_blood_issuance_product_id
