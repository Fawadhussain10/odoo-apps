...
class PBSurgeryTemplate(models.Model):
...
class PBSurgery(models.Model):
    ...
    def pb_get_consume_locations(self):
        if not self.company_id.pb_surgery_usage_location_id:
            raise UserError(_('Please define a location where the consumables will be used in settings.'))
        if not self.company_id.pb_surgery_stock_location_id:
            raise UserError(_('Please define a surgery location from where the consumables will be taken.'))
        source_location_id  = self.company_id.pb_surgery_stock_location_id.id
        dest_location_id  = self.company_id.pb_surgery_usage_location_id.id
    ...
    def action_create_invoice(self):
        ...
        pb_context = {'commission_partner_ids':self.primary_physician_id.partner_id.id}
