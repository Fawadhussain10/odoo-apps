...
class PbHmsPackage(models.Model):
    _name = "pb.hms.package"
    ...
    order_line = fields.One2many("pb.hms.package.line", 'order_id', string='Order Lines', readonly=False, copy=True)
...
class PbHmsPackageLine(models.Model):
    _name = 'pb.hms.package.line'
    ...
    order_id = fields.Many2one('pb.hms.package', string='Order', required=True, ondelete='cascade', index=True, copy=False)
    ...
    def product_id_change(self):
        ...
        vals['price_unit'] = product.with_context(pb_pricelist_id=self.order_id.pricelist_id.id)._pb_get_partner_price()
        vals['name'] = name
        self.update(vals)
