...
class PBHms(http.Controller):
    ...
    def labresult_details(self, labresult_unique_code, **post):
        if labresult_unique_code:
            ...
            if labresult:
                return request.render("pb_radiology.pb_radiology_result_details", {'labresult': labresult})
        return request.render("pb_hms.pb_no_details")
...
class HMSPortal(CustomerPortal):
    ...
    def my_radiology_results(self, page=1, sortby=None, **kw):
        ...
        return request.render("pb_radiology.radiology_results", values)
    ...
    def my_radiology_test_result(self, result_id=None, access_token=None, **kw):
        ...
        if order_sudo.attachment_ids:
            for att in order_sudo.attachment_ids:
                att._pb_portal_ensure_token()

        return request.render("pb_radiology.my_radiology_test_result", {'radiology_result': order_sudo})
    ...
    def my_radiology_requests(self, page=1, sortby=None, **kw):
        values = self._prepare_portal_layout_values()
        RadiologyReq = request.env['pb.radiology.request']
        if not sortby:
            sortby = 'date'
        ...
        return request.render("pb_radiology.radiology_requests", values)
    ...
    def my_radiology_test_request(self, radiology_request_id=None, access_token=None, **kw):
        try:
            order_sudo = self._document_check_access('pb.radiology.request', radiology_request_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        return request.render("pb_radiology.my_radiology_test_request", {'radiology_request': order_sudo})
