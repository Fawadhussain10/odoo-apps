...
class PatientLabTestLine(models.Model):
    ...
    def _pb_is_manager(self):
        is_manager = self.env.user.has_group('pb_radiology.group_hms_radiology_manager')
        for rec in self:
            rec.is_manager = is_manager

    test_id = fields.Many2one('pb.radiology.test',string='Test', ondelete='cascade', required=True)
    ...
    radiology_request_id = fields.Many2one('pb.radiology.request',string='Lines', ondelete='cascade')
...
class RadiologyRequest(models.Model):
    ...
    pb_radiology_invoice_policy = fields.Selection(related="company_id.pb_radiology_invoice_policy")
    ...
    def button_requested(self):
        ...
        self.name = self.env['ir.sequence'].next_by_code('pb.radiology.request')
    ...
    def button_accept(self):
        ...
        if company_id.pb_radiology_invoice_policy=='in_advance':
            if not self.invoice_id:
                raise UserError(_('Invoice is not created yet.'))
            elif self.invoice_id and company_id.pb_check_radiology_payment and self.payment_state not in ['in_payment','paid']:
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(pb_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data={'hospital_invoice_type': 'radiology','physician_id': self.physician_id and self.physician_id.id or False})
    ...
    def action_view_test_results(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_radiology.action_radiology_result")
