...
class PatientLabTest(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'pb.hms.mixin', 'portal.mixin', 'pb.documnt.mixin', 'pb.qrcode.mixin', 'pb.documnt.view.mixin']
    _description = "Patient Radiology Test"
    _order = 'date_analysis desc, id desc'

    @api.model
    def _get_disclaimer(self):
        return self.env.user.sudo().company_id.pb_radiology_disclaimer or ''

    STATES = {'cancel': [('readonly', True)], 'done': [('readonly', True)]}

    name = fields.Char(string='Test ID', help="Radiology result ID", readonly=True,copy=False, index=True, tracking=True)
    test_id = fields.Many2one('pb.radiology.test', string='Test', required=True, ondelete='restrict', tracking=True)
    ...
    radiology_request_id = fields.Many2one('pb.radiology.request', string='Test Request', ondelete='restrict')
    #pbv16: cehck and remove
    #radiology_id = fields.Many2one('pb.radiology', related="radiology_request_id.radiology_id", string='Radiology', readonly=True, store=True)
    ...
    def _track_log_get_default_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'done':
            return self.env.ref('pb_radiology.mt_radiology_test_done')
    ...
    def _subscribe_physician(self):
        done_subtype = self.env.ref('pb_radiology.mt_radiology_test_done').id
    ...
    def action_radiology_test_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('pb_radiology.pb_radiology_test_email', raise_if_not_found=False)
    ...
    def pb_get_consume_locations(self):
        if not self.company_id.pb_radiology_usage_location_id:
            raise UserError(_('Please define a location where the consumables will be used during the Radiology test in company.'))
        if not self.company_id.pb_radiology_stock_location_id:
            raise UserError(_('Please define a Radiology location from where the consumables will be taken.'))

        dest_location_id  = self.company_id.pb_radiology_usage_location_id.id
        source_location_id  = self.company_id.pb_radiology_stock_location_id.id
