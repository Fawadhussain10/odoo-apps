...
class AccountInvoice(models.Model):
    ...
    radiology_request_id = fields.Many2one('pb.radiology.request', string='Radiology Request', copy=False, ondelete='restrict')
...
class PBConsumableLine(models.Model):
    ...
    radiology_test_id = fields.Many2one('pb.radiology.test', string="Radiology Test", ondelete="restrict")
...
class PBPatient(models.Model):
    ...
    radiology_request_ids = fields.One2many('pb.radiology.request', 'patient_id', string='Radiology Requests')
    ...
    def action_radiology_requests(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_radiology.hms_action_radiology_request")
...
class Physician(models.Model):
    ...
    def _pb_rec_radiology_count(self):
        Labrequest = self.env['pb.radiology.request']
    ...
    radiology_request_count = fields.Integer(compute='_pb_rec_radiology_count', string='# Radiology Request')
    radiology_result_count = fields.Integer(compute='_pb_rec_radiology_count', string='# Radiology Result')
