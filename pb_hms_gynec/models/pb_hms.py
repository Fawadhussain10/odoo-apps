...
class PBAppointment(models.Model):
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        if context.get('pb_department_type')=='gynec':
...
class PBPatient (models.Model):
    ...
    child_birth_ids = fields.One2many('pb.child.birth', 'patient_id',string='Child Birth')
    ...
    delivery_ids = fields.One2many('pb.child.birth', 'patient_id',string='Delivery')
    ...
    def action_view_patient_delivery(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_gynec.hms_action_form_delivery")
...
class PBInpatientRegistration(models.Model):
    _inherit = "pb.hospitalization"

    delivery_ids = fields.One2many('pb.child.birth', 'hospitalizaion_id',string='Delivery')
