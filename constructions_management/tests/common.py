...
class ConstructionTestCommon(TransactionCase):
    """Shared fixtures for constructions_management tests. Each test method runs inside its own
    savepoint (TransactionCase default), so nothing here needs manual cleanup."""

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.category = cls.env.ref("constructions_management.project_category_construction")
        cls.subcategory = cls.env.ref("constructions_management.project_subcategory_house")
