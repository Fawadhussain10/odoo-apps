...
class PaymentAllocationLines(models.TransientModel):
    ...
    def _calc_amount_residual(self):
        ...
        for record in self:
            max_date = max(
                record.allocation_id.line_ids.filtered('allocate').mapped('move_line_id.date') or [fields.Date.today()])
            ...
            if record.allocation_currency_id == record.company_currency_id:
                ...
                record.amount_residual = record.move_currency_id.with_context(
                    manual_currency_rate=manual_currency_rate)._convert(record.move_line_id.amount_residual_currency,
                                                                        record.allocation_currency_id,
                                                                        record.allocation_id.company_id, max_date)
            else:
                record.amount_residual = record.company_currency_id.with_context(
                    manual_currency_rate=manual_currency_rate)._convert(record.move_line_id.amount_residual,
                                                                        record.allocation_currency_id,
                                                                        record.allocation_id.company_id, max_date)
    ...
    @api.onchange('allocate', 'allocate_amount')
    def set_validation_on_amount_allocation(self):
        for rec in self:
            if rec.allocate and rec.allocate_amount:
                if rec.allocate_amount > rec.amount_residual_display:
                    raise ValidationError(
                        _(f"Invoice {rec.move_name} allocate amount {round(rec.allocate_amount, 3)} cannot be greater than unallocate amount {round(rec.amount_residual_display, 3)}."))
                if rec.allocate_amount <= 0:
                    raise ValidationError(
                        _(f"Invoice {rec.move_name} allocate amount {round(rec.allocate_amount, 3)} cannot be less than or equal to zero."))
