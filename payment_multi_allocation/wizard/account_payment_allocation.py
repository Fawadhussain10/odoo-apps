...
class PaymentAllocation(models.TransientModel):
    ...
    def _calc_manual_currency_rate(self):
        ...
        payment_line_ids = line_ids.filtered(
            lambda line: line.allocate and line.payment_id and line.move_currency_id != line.company_currency_id)
        ...
        if not payment_line_ids:
            refund_line_ids = line_ids.filtered(
                lambda line: line.allocate and line.invoice_id.move_type in ['in_refund',
                                                                             'out_refund'] and line.move_currency_id != line.company_currency_id)
    ...
    def default_get(self, fields_list):
        if self._context.get("active_model") == 'account.move' and self.env['ir.config_parameter'].sudo().get_param(
                'payment.allocation.invoice_disabled') == 'True':
    ...
    def _check_move_line_ids(self):
        ...
        for line in self.move_line_ids:
            ...
            if not line.account_id.reconcile and line.account_id.account_type not in ('asset_cash',
    ...
    def _onchange_invoice_ids(self):
        if self.invoice_ids:
            self.account_id = self.invoice_ids.line_ids.filtered(
                lambda line: not line.reconciled and line.account_id.reconcile).account_id[:1]
    ...
    def validate(self):
        for rec in self.line_ids:
            rec.set_validation_on_amount_allocation()
        ...
        if self.writeoff_journal_id and self.writeoff_line_ids:
            ...
            if self.currency_id != self.company_id.currency_id:
                ...
                balance = self.currency_id._convert(amount_currency, self.company_id.currency_id, self.company_id,
                                                    max_date)
            else:
                currency_id = False
            ...
            move_vals['line_ids'].append(Command.create({
                ...
                'partner_id': self.partner_id.id,
                'display_type': 'payment_term' if self.account_id.account_type in ['asset_receivable',
                                                                                   'liability_payable'] else 'product'
            }))
        ...
        if round(sum(debit_line_ids.mapped('allocate_amount'), 3)) != round(
                sum(credit_line_ids.mapped('allocate_amount'), 3)):
            raise ValidationError(_("Debit amount must be equal to credit amount."))
        ...
        for debit_line in debit_line_ids:
            for credit_line in credit_line_ids:
                ...
                vals = {
                    ...
                    'amount': self.currency_id._convert(allocate_amount, self.company_id.currency_id, self.company_id,
                                                        max_date),
                    'debit_amount_currency': self.currency_id._convert(allocate_amount,
                                                                       debit_line.move_line_id.currency_id,
                                                                       self.company_id, max_date),
                    'credit_amount_currency': self.currency_id._convert(allocate_amount,
                                                                        credit_line.move_line_id.currency_id,
                                                                        self.company_id, max_date)
                }

                for n in range(2):
                    for line, amount_currency, sign in [(debit_line, vals['debit_amount_currency'], 1),
                                                        (credit_line, vals['credit_amount_currency'], -1)]:
                        if line.move_line_id.amount_currency:
                            ...
                            if n == 0 and (diff_amount * sign) < 0:
                                vals['amount'] = self.company_id.currency_id.round(
                                    vals['amount'] + (diff_amount * sign))
                            if n == 1:
        ...
        for move_line in move_line_ids:
            ...
            if not move_line.amount_residual_currency and move_line.amount_residual:
                ...
                amount = move_line.amount_residual - move_line.currency_id._convert(move_line.amount_residual_currency,
        ...
        if exchange_lines:
            exchange_move = self.env['account.move'].with_context(skip_invoice_sync=True).create(
                self._prepare_exchange_diff_move(move_date=max_date))
            exchange_journal = exchange_move.journal_id
            for move_line, amount in exchange_lines:
                line_to_rec = self.env['account.move.line'].with_context(check_move_validity=False,
                ...
                if (move_line.id == exchange_partial_vals['debit_move_id'] and move_line.credit) or (
                        move_line.id == exchange_partial_vals['credit_move_id'] and move_line.debit):
        ...
        if reconciled_move_line_ids:
            partial_reconcile_ids = partial_reconcile_ids.filtered(lambda
                                                                       record: record.debit_move_id in reconciled_move_line_ids or record.credit_move_id in reconciled_move_line_ids) + exchange_partial_reconcile
    ...
    def validate_test(self):
        # Debugging: Ensure values are coming through for debit_line_ids and credit_line_ids
        for line in self.line_ids:
            print(f"Line ID: {line.id}, Allocate: {line.allocate}, Allocate Amount: {line.allocate_amount}")

        # Ensure `allocate_amount` is preserved before validation
        for debit_line in self.debit_line_ids:
            if debit_line.allocate and debit_line.allocate_amount:
                print(f"Debit Line: {debit_line.id}, Allocate Amount: {debit_line.allocate_amount}")
                debit_line.allocate_amount = debit_line.allocate_amount  # Preserve value

        for credit_line in self.credit_line_ids:
            if credit_line.allocate and credit_line.allocate_amount:
                print(f"Credit Line: {credit_line.id}, Allocate Amount: {credit_line.allocate_amount}")
                credit_line.allocate_amount = credit_line.allocate_amount  # Preserve value
        ...
        # Debugging: Check the filtered line IDs
        print(f"Debit Line IDs: {[line.id for line in debit_line_ids]}")
        print(f"Credit Line IDs: {[line.id for line in credit_line_ids]}")
...
class AccountFullReconcile(models.Model):
    _inherit = 'account.full.reconcile'  # Inherit the base model

    name = fields.Char(string='Reconcile Name')  # Define the 'name' field
