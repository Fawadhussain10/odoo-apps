...
class PbVideoCall(models.Model):
    ...
    def _get_meeting_link(self):
        on_own_site = self.env['ir.config_parameter'].sudo().get_param('pb_jitsi_meet.call_on_own_site')
        if on_own_site:
            server_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url') + '/videocall'
        else:
            server_url = self.env['ir.config_parameter'].sudo().get_param('pb_jitsi_meet.video_call_server_url')
        for rec in self:
