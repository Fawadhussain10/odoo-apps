...
class PbVideoCall(http.Controller):
    ...
    def pb_videocall(self, meeting_name, **kw):
        videocall_server_url = request.env['ir.config_parameter'].sudo().get_str('pb_jitsi_meet.video_call_server_url')
        site_url = request.env['ir.config_parameter'].sudo().get_str('web.base.url')
