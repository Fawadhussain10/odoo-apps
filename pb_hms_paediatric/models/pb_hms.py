...
class Appointment(models.Model):
    ...
    pb_head_circum_name = fields.Char(related="evaluation_id.pb_head_circum_name", string='Patient Head Circumference unit of measure label')
...
class PBPatient (models.Model):
    ...
    def get_line_graph_datas(self, record_type):
        ...
        normal_data = []
        child_data = []
        normal_record_datas = self.env['pb.growth.chart.data'].search([('record_type','=',record_type)])
        ...
        for normal_record_data in normal_record_datas:
            ...
            child_record_data = self.env['pb.patient.evaluation'].search([('patient_id','=',self.id), ('age_month','=',int(normal_record_data.age))], limit=1)
    ...
    pb_head_circum_name = fields.Char(related="last_evaluation_id.pb_head_circum_name", string='Patient Head Circumference unit of measure label')

    def show_growth_chart(self):
        action = self.env["ir.actions.actions"]._for_xml_id("pb_hms_paediatric.action_growth_chart")
