...
class TestFbrIntegration(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.company = company
        company.write({'fbr_token': 'test-token', 'fbr_mode': 'sandbox'})
    ...
    def test_company_settings_defaults(self):
        company = self.env['res.company'].create({'name': 'Second FBR Company'})
        self.assertEqual(company.fbr_mode, 'sandbox')
        self.assertEqual(company.fbr_bpos_id, '05')
        self.assertFalse(company.fbr_enable_service)
