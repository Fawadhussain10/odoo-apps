...
class PbHmsWaitingScreen(models.Model):
    _name = 'pb.hms.waiting.screen'
    ...
    pb_refresh_time = fields.Integer("Refresh Time (Seconds)", default=5)
    pb_number_of_records = fields.Integer("Shown Number of Records", default=5)
    ...
    pb_physician_field_id = fields.Many2one('ir.model.fields','Physician', ondelete='cascade', domain="['&',('ttype', '=', 'many2one'),('model_id','=',res_model_id)]", required=True,
        help="Physician Field shown on waiting screen: If no physician in object user can be also shown.")
    pb_patient_field_id = fields.Many2one('ir.model.fields','Patient', ondelete='cascade', domain="['&',('ttype', '=', 'many2one'),('model_id','=',res_model_id)]")
    pb_cabin_field_id = fields.Many2one('ir.model.fields','Cabin', ondelete='cascade', domain="['&',('ttype', '=', 'many2one'),('model_id','=',res_model_id)]",
        help="Selected field data willbe shown on cabin column in waiting screen.")
    pb_state_field_id = fields.Many2one('ir.model.fields','State', ondelete='cascade', domain="['&',('ttype', '=', 'selection'),('model_id','=',res_model_id)]", required=True,
        help="On Selected field domain will be appliend on record to be shown in waiting screen.")
    pb_states_to_include = fields.Char(string="Records with States to Include in WC", default="['waiting','in_consultation']",
    ...
    def on_change_object(self):
        ...
        if self.res_model_id:
            ...
            self.pb_physician_field_id = self.pb_patient_field_id = self.pb_cabin_field_id = self.pb_state_field_id = None
            self.pb_states_to_include = '[]'

            physician_field = IMFObj.search([("model","=",self.res_model_id.model),("name","=","physician_id")], limit=1)
            if physician_field:
                self.pb_physician_field_id = physician_field.id

            patient_field = IMFObj.search([("model","=",self.res_model_id.model),("name","=","patient_id")], limit=1)
            if patient_field:
                self.pb_patient_field_id = patient_field.id

            cabin_field = IMFObj.search([("model","=",self.res_model_id.model),("name","=","cabin_id")], limit=1)
            if cabin_field:
                self.pb_cabin_field_id = cabin_field.id
            else:
                self.show_cabin = False

            state_field = IMFObj.search([("model","=",self.res_model_id.model),("name","=","state")], limit=1)
            if state_field:
                self.pb_state_field_id = state_field.id

            if self.res_model_id.model=='hms.appointment':
                self.pb_states_to_include = "['waiting','in_consultation']"
            if self.res_model_id.model=='pb.laboratory.request':
                self.pb_states_to_include = "['draft','requested']"
            if self.res_model_id.model in ['pb.laboratory.request','pb.patient.laboratory.sample']:
                self.pb_states_to_include = "['draft']"

    def pb_open_website_url(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_url',
            'url': '/pb/waitingscreen/' + str(self.id),
            'target': 'new',
        }
