...
class HMSWebsite(Website):
    ...
    def user_booking_data(self, post):
        ...
        values.update({
            'terms_page_link': user.company_id.pb_appointment_tc,
            'allow_video_call': user.company_id.pb_allowed_video_consultation,
    ...
    def create_appointment(self, redirect=None, **post):
        ...
        return request.render("pb_hms_online_appointment.appointment_details", values)
    ...
    def create_appointment_data(self, redirect=None, **post):
        ...
        return request.render("pb_hms_online_appointment.appointment_slot_details", values)
    ...
    def appointment_personal_data(self, redirect=None, **post):
        values = self.user_booking_data(post)
        return request.render("pb_hms_online_appointment.appointment_personal_details", values)
    ...
    def save_appointment(self, redirect=None, **post):
        ...
        if post:
            ...
            post.pop('pb_appointment_tc', '')

            # Create Appointment
            app_id = app_obj.sudo().create(post)

            if user.sudo().company_id.allowed_booking_payment:
                app_id.sudo().with_context(pb_online_transaction=True).onchange_physician()
                ...
                if payment_link_wiz.amount==0:
                    app_id.sudo().with_context(pb_online_transaction=True).appointment_confirm()
                    return request.render("pb_hms_online_appointment.appointment_thank_you", {'appointment': app_id})
...
class PbCustomerPortal(CustomerPortal):
    ...
    def _get_custom_rendering_context_values(self, **kwargs):
        ...
        :rtype: dict
        """
        res = super(PbCustomerPortal, self)._get_custom_rendering_context_values(**kwargs)
        if kwargs.get('pb_appointment_id'):
            res['pb_appointment_id'] = kwargs.get('pb_appointment_id')
        return res

    def _create_transaction(
        self, payment_option_id, reference_prefix, amount, currency_id, partner_id, flow,
        tokenization_requested, landing_route, is_validation=False, invoice_id=None, pb_appointment_id=None,
        custom_create_values=None, **kwargs
    ):
        res = super(PbCustomerPortal, self)._create_transaction(payment_option_id, reference_prefix, amount, currency_id, partner_id, flow,
        ...
        if pb_appointment_id:
            res.pb_appointment_id = pb_appointment_id
        return res
