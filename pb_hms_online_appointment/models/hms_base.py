...
class Appointment(models.Model):
    ...
    def pb_get_slot_lines(self, physician_id, department_id, date, schedule_type):
        if date:
    ...
    def get_slot_data(self, physician_id, department_id, date=False, schedule_type="appointment"):
        slot_lines = self.pb_get_slot_lines(physician_id, department_id, date, schedule_type)
        slot_data = []
    ...
    def _get_default_payment_link_values(self):
        ...
        amount = self.pb_get_total_amount(product_data, self.patient_id.partner_id)
        return {
    ...
    def pb_get_total_amount(self, product_data, partner):
        total_amount = 0
        for data in product_data:
            product = data.get('product_id')
            if product:
                pb_pricelist_id = self.env.context.get('pb_pricelist_id')
                if not data.get('price_unit') and (partner.property_product_pricelist or pb_pricelist_id):
                    if pb_pricelist_id:
...
class PaymentTransaction(models.Model):
    ...
    pb_appointment_id = fields.Many2one("hms.appointment", string="Appointment")

    #Invoice and confirm the appointment after a successful payment, before
    #the payment post-processing reconciles the transaction's invoices.
    def _post_process(self):
        for tx in self.filtered(lambda t: t.state == 'done' and t.operation != 'validation' and t.pb_appointment_id and not t.invoice_ids):
            tx._pb_update_apppointment()
        return super()._post_process()

    #Update appointment data.
    def _pb_update_apppointment(self):
        self.ensure_one()
        self.pb_appointment_id.sudo().with_context(pb_online_transaction=True,default_create_stock_moves=False).create_invoice()
        if self.pb_appointment_id.sudo().state!='confirm':
            self.pb_appointment_id.sudo().with_context(pb_online_transaction=True).appointment_confirm()
        ...
        self.pb_appointment_id.invoice_id._portal_ensure_token()
        self.invoice_ids = [(6, 0, [self.pb_appointment_id.invoice_id.id])]
