...
class PBPhysiotherapyExercise(models.Model):
...
class PBPhysiotherapyExerciseGroup(models.Model):
    ...
    _code_uniq = models.Constraint('unique (name)', 'Physiotherapy Group Name must be unique')


class PBPhysiotherapyExerciseGroupLines(models.Model):
    ...
    physiotherapy_id = fields.Many2one('pb.physiotherapy','Physiotherapy')
...
class PBPhysiotherapyNoteTemplate(models.Model):
    _name = "pb.physiotherapy.note.template"
...
class PBPhysiotherapyNote(models.Model):
    _name = "pb.physiotherapy.note"
    _description = "Physiotherapy Note"
    # _order = sequence
    ...
    sensory_val = fields.Char("Sensory")
    note_type = fields.Selection([('lower','Lower'),('upper','Upper'),('hand','Hand')],string="Note Type",required=True)
    hand_data_id = fields.Many2one('pb.physiotherapy','Note')
    lower_data_id = fields.Many2one('pb.physiotherapy','Lower Note')
    upper_data_id = fields.Many2one('pb.physiotherapy','Upper Note')


#master Template data
class PBPhysiotherapySelectionNoteTemplate(models.Model):
    _name = "pb.physiotherapy.selection.note.template"
...
#master Template data
class PBPhysiotherapySelectionNote(models.Model):
    _name = "pb.physiotherapy.selection.note"
    _description = "Physiotherapy Selection Note"
    # _order = sequence
    ...
    left_val = fields.Selection([('normal', 'Normal'),('other', 'Other'),('positive', 'Positive'),('negative', 'Negative')],string="STRENGTH LEFT",default='normal')
    selnote_type = fields.Selection([('lower','Lower'),('upper','Upper'),('hand','Hand')],string="Note Type")
    lower_selectdata_id = fields.Many2one('pb.physiotherapy','Lower SNote')
    upper_selectdata_id = fields.Many2one('pb.physiotherapy','Upper SNote')


class PBPhysiotherapy(models.Model):
    _name = "pb.physiotherapy"
    _description = "Physiotherapy"
    _inherit = ['pb.hms.mixin']
    ...
    patient_id = fields.Many2one('hms.patient', string='Patient',required=True, index=True, help='Patient Name')
    ...
    hand_data_ids =  fields.One2many('pb.physiotherapy.note','hand_data_id',string='Hand Note')
    lower_data_ids =  fields.One2many('pb.physiotherapy.note','lower_data_id',string='Lower Note')
    upper_data_ids =  fields.One2many('pb.physiotherapy.note','upper_data_id',string='Upper Note')
    lower_seldata_ids =  fields.One2many('pb.physiotherapy.selection.note','lower_selectdata_id',string='Lower Selection Note')
    upper_seldata_ids =  fields.One2many('pb.physiotherapy.selection.note','upper_selectdata_id',string='Upper Selection Note')
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(pb_context).pb_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data={'hospital_invoice_type': 'physiotherapy'})
    ...
    def default_get(self, fields):
        res = super(PBPhysiotherapy, self).default_get(fields)
        vals = []
        hand_templates = self.env['pb.physiotherapy.note.template'].search([('note_type','=','hand')])
        ...
        vals = []
        lower_templates = self.env['pb.physiotherapy.note.template'].search([('note_type','=','lower')])
        ...
        upper_templates = self.env['pb.physiotherapy.note.template'].search([('note_type','=','upper')])
        ...
        vals = []
        lower_selection_templates = self.env['pb.physiotherapy.selection.note.template'].search([('selnote_type','=','lower')])
        ...
        vals = []
        upper_selection_templates = self.env['pb.physiotherapy.selection.note.template'].search([('selnote_type','=','upper')])
